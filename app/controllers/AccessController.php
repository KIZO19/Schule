...
class AccessController extends Controller {
    ...
    public function login() {
        ...
        // Si deja connecte, rediriger selon le type present en session
            ...
            if (!$this->verifyCsrfToken($csrf)) {
                ...
                if ($this->isRateLimited($rateKey, 6, 300)) {
                    ...
                    if (!$user) {
                        ...
                        // On a trouve un enregistrement dans `utilisateurs`.
                        if (!isset($user['mot_de_passe']) || !password_verify($password, $user['mot_de_passe'])) {
                            ...
                            // Charger les donnees de reference selon le role
                            ...
                            switch ($user['role']) {
                                ...
                                case 'DP_ecole':
                                case 'DA_ecole':
                                case 'comptable_ecole':
                                case 'sec_ecole':
                                case 'promoteur_ecole':
                                case 'enseignant_ecole':
                                    // Agent-like roles
                                    if ($ref) {
                                ...
                                case 'eleve_ecole':
                                    if ($ref) {
                                        // Connecter comme eleve: on pourrait stocker eleve_id
